 ...
 */
angular.module('standard').directive('appViewSource', function($rootScope, $templateCache, $http) {
  ...
  return {
    ...
    link: function(scope, element, attrs) {
      ...
      $rootScope.$on('$stateChangeSuccess', function(event, toState, toParams, fromState, fromParams) {
        ...
        var cssFile = toState.controller.replace(/^ctrl\./, '').replace(/\./g, '/') + '.css';
        $http.get("/source/standard/css/"+cssFile).success(function(data) {
          mainCss.source = data;
        });
        
        var fileName = toState.controller.replace(/^ctrl\./, '').replace(/\./g, '/') + '.js';
        $http.get("/source/standard/controllers/" + fileName).success(function(data) {
          mainJs.source = data;
        });
